...
from .layer_map import LOGICAL_LAYERS, LOGICAL_LAYER_VERSION, SPLIT_SUMMARY


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Create M3 logical L0-L16 contracts for unknown CSV/JSON/JSONL sources.")
    ...
    parser.add_argument("--output", required=True, help="Output directory for M3 contract artifacts. Physical folders remain l0-l10 for compatibility; logical layers are L0-L16.")
    parser.add_argument("--run-id", default="m3_contract_run_001")
    parser.add_argument("--format", choices=["auto", "csv", "json", "jsonl", "parquet", "text"], default="auto")
    ...
    parser.add_argument(
        ...
        default="deferred",
        help="Logical L9 user decision for Gold. Default keeps Gold recommendation visible but not executable.",
    )
...
def run(args: argparse.Namespace) -> dict[str, object]:
    ...
    summary = {
        ...
        "run_id": args.run_id,
        "output": str(out_dir),
        "layer_contract_version": LOGICAL_LAYER_VERSION,
        ...
        "m3_scope": "Logical L0-L16 contract planning, AI-safe recommendation, preview-only deterministic specs, and catalog/query handoff only; no distributed runtime ownership, no production write, no raw copy",
        "data_shape_contract": l2["profile"].get("data_shape_contract", {}),
        "handoff": {
            "M1": "source registration and L5 decision UI",
            "M2": "execute L10 Silver and L11 Gold preview-only specs, then return L13/L14 preview metrics",
            "M5": "store L9 approvals and L16 catalog sync package/run/catalog state",
            "M6": "use L16 SQL/query/vector context with L15 caveats",
        },
