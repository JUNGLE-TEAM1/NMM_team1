...
class QueryRouter:
    ...
    def decide(self, question: str, plan: SqlPlan) -> QueryRouteDecision:
        ...
        if has_sql_route and has_rag_route:
            return QueryRouteDecision(route="hybrid", requires_sql=True, uses_rag=True, reason_terms=rag_terms)
        if has_sql_route:
            return QueryRouteDecision(route="sql", requires_sql=True, uses_rag=False)
        if has_rag_route:
            return QueryRouteDecision(route="rag", requires_sql=False, uses_rag=True, reason_terms=rag_terms)
        return self._unsupported(plan)
