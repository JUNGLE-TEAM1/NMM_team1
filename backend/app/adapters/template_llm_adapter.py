...
class TemplateLLMAdapter:
    ...
    def generate_summary(self, context: LLMAnswerContext) -> LLMAnswer:
        ...
        if context.route == "hybrid":
            return LLMAnswer(
                summary=f"{self._grounded_summary(answer, evidence)} SQL 결과와 CatalogMetadata 근거를 함께 사용했습니다.",
                source="template",
                used_evidence_indexes=[0],
            )
        return LLMAnswer(summary=answer, source="template", used_evidence_indexes=[0])
