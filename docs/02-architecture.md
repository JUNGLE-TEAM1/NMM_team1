...
| Data | PostgreSQL or SQLite metadata + container/local output candidate | MVP는 운영형 데이터 레이크보다 container-first로 재현 가능한 저장소를 우선한다. |
...
- 책임: 실제 제품 기능을 구현한다.
- 주요 파일/모듈: 향후 `frontend/`, `backend/`, local runner 모듈.
...
| Docker registry / AWS ECR 후보 | image 저장 | image tag/rollback tag 기록 |
| Kubernetes / AWS EKS 후보 | 배포 runtime | smoke 실패 시 rollout 중단 또는 rollback |

## 7) 장기 컴포넌트 후보

| 컴포넌트 | Local-first | Single-node | Optional distributed/cloud |
| --- | --- | --- | --- |
| Source connector | CSV/local file, PostgreSQL 후보 | PostgreSQL/MySQL/MongoDB/REST API 후보 | S3, managed DB, CDC source |
| Pipeline runner | FastAPI 내부 또는 local worker | background worker, retry/log 저장 | Spark, Airflow DAG |
| Result storage | local file, SQLite/PostgreSQL table | PostgreSQL 또는 container volume | S3/data lake |
| Catalog/search | DB query와 basic search | indexed metadata table | OpenSearch/Nori |
...
| AWS | 계정/region/IAM/ECR/EKS 또는 대체 compute/storage 전략 | 비용 발생 resource 생성 전 approval checklist |
...
| Product runtime env | TBD | MVP 코드 생성 후 `.env.example`에 기록 |
...
- Migration command: TBD after metadata store decision.
