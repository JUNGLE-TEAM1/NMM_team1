...
| Result storage | local CSV `ResultStore`, Week 2 local fallback storage adapter | result dataset metadata가 catalog에 연결. Week 2는 S3-compatible URI와 local fallback path를 같은 prefix로 계산 |
