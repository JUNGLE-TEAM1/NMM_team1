...
- [ ] MongoDB External Connection을 저장 전 테스트할 수 있고, metadata를 저장/조회할 때 password 원문은 저장하거나 응답하지 않는다.
- [ ] 저장된 MongoDB External Connection에서 `database.collection` schema preview를 조회해 Source Dataset 생성에 사용할 수 있다.
...
- [ ] Target Dataset 처리 계획 화면은 `추천 템플릿 사용`과 `직접 설정` mode를 구분하고, Product Health 추천 템플릿 선택 시 Source 역할, 내부 처리 단계, 최종 Gold Target output, `quality_rules`를 review할 수 있다.
- [ ] Product Health Transform Builder MVP는 추천 template을 자동 적용하고 role별 column mapping을 metadata로 저장하며 normalize, aggregate metrics, join key, `risk_score`, Gold schema는 review-only 또는 locked로 표시한다.
...
- [ ] Product Health 추천 템플릿으로 저장한 Target Dataset `process_rule`에는 `type=product_health_gold_pipeline`, `input_kind=raw_sources`, `final_output`, `internal_artifacts_visible=false`, `template_id`, `template_version`, `target_dataset`, `query_table`, `steps[]`, `builder_config`, `quality_rules[]`, `output_schema[]`가 포함된다.
...
- [ ] Product Health Target Dataset 수동 실행은 실제 Gold 생성 전이라도 `product_health_manual_run_contract`를 반환해 source snapshot input, pending Gold output, pending quality results, lineage, `catalog_payload` handoff shape를 고정한다.
