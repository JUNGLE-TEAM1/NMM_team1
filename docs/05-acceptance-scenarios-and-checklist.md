...
- [ ] Target Dataset Pipeline Plan 화면은 `추천 템플릿 사용`과 `직접 설정` mode를 구분하고, Product Health 추천 템플릿 선택 시 raw source roles, 내부 처리 단계, 최종 Gold Target output, `quality_rules`를 review할 수 있다.
...
- [ ] 저장된 Target Dataset에서 Manual Run smoke를 생성하고 `week2_run_id`, executor, status를 확인할 수 있다.
- [ ] Target Dataset Manual Run smoke는 `ExecutionResult`와 `target_dataset_handoff`를 함께 보존하며, C-4 fixture-backed smoke output이면 `runtime_output_scope=week2_fixture_output`를 표시한다.
