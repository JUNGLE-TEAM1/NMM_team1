...
- 열림:
  - M3에서 file upload UI까지 포함할지, repo/container 내부 sample path 등록으로 제한할지 결정해야 한다.
  - M4에서 pipeline result storage를 local file, SQLite table, 또는 container volume 중 어디까지 쓸지 결정해야 한다.
