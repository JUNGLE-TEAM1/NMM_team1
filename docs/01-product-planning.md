...
AskLake는 여러 데이터 소스를 연결해 대용량/복합 데이터셋을 수집·스키마화·변환·검산·게시하고, 권한 안에서 SQL과 자연어 질문으로 활용하며, 답변과 분석 결과의 근거를 리니지·품질·정책·감사 기록으로 확인하게 하는 B2B SaaS Trusted Data & AI Platform이다.
...
- 대용량/복합 데이터셋은 가져오는 것만으로는 부족하며, schema inference, transform/normalize, 저장 위치, 처리 규모, SQL 검산 evidence가 함께 남아야 신뢰 가능한 분석 자산이 된다.
- SQL에서는 차단된 데이터가 AI 답변에는 노출되거나, 파이프라인은 성공했지만 품질 정책 때문에 게시하면 안 되는 데이터가 소비될 수 있다.

AskLake의 핵심 가설은 데이터 수집·가공부터 Query/Ask 답변, 장애 복구까지 같은 메타데이터, 정책, 리니지, 처리 증거, 감사 모델을 공유하면 작은 팀도 “왜 이 결과를 믿을 수 있는가”를 설명할 수 있다는 것이다.
...
2. schema inference 또는 user override를 거쳐 transform/normalize/load를 실행한다.
3. output path, row count, bytes, duration, SQL 검산 evidence를 남긴다.
4. catalog draft와 품질/PII/소유자/정책 정보를 연결한다.
5. 필수 gate를 통과한 데이터셋만 `Trusted`로 게시한다.
6. Query 또는 Ask에서 권한 preflight와 정책을 적용한다.
7. 답변 또는 분석 결과에 SQL, 데이터셋, 지표, 문서, freshness, lineage, policy decision evidence를 연결한다.
8. schema drift나 품질 실패가 발생하면 영향 자산을 확인하고 retry/rerun/backfill로 복구한다.
...
- Dataset manipulation: schema inference/user override, transform 또는 normalize, load를 통해 대용량/복합 데이터셋을 분석 가능한 output dataset으로 만든다.
- Catalog draft: source 또는 pipeline output에서 dataset draft를 만들고 schema, owner, freshness, lineage seed를 연결한다.
- Processing evidence: output path, row count, bytes, duration, SQL 검산 결과를 dataset evidence로 남긴다.
...
- production-grade distributed processing과 full-scale 성능 benchmark
...
| Target MVP | 다음에 증명할 신뢰 루프 | 대용량/복합 dataset manipulation evidence를 포함한 `Trusted Dataset -> Query/Ask -> Evidence -> Recovery` |
...
| R3. Source 확장 | Source Connector | baseline 외 source 하나를 선택해 연결한다 | PostgreSQL 또는 REST API 중 하나, schema discovery | 모든 source 동시 지원 | 연결 성공/실패와 schema discovery 확인 |
...
- 대용량/복합 데이터셋의 schema inference, transform/normalize/load, output path, row count, bytes, duration, SQL 검산 evidence를 설명할 수 있다.
