...
- 데이터 소스 등록: MVP는 CSV/local file source를 등록한다. PostgreSQL 또는 MySQL은 M6에서 확장한다.
- 파이프라인 작성: source -> transform -> target 흐름을 UI나 간단한 form으로 정의한다.
- 기본 변환: MVP는 `select_fields` 기반 컬럼 선택을 지원한다. row filter는 M7 변환 확장 후보로 둔다.
...
| M3. 소스와 카탈로그 | CSV/local file source 등록, 교체 가능한 metadata store 경계, catalog 목록/상세 표시 | 등록한 CSV source가 카탈로그에 보이고 schema/row count/sample이 표시됨 | SQLite 기반 metadata store 구현, `MetadataStore` 경계 기록, source 등록/list/detail과 catalog list/detail API 확인, 연결 실패 메시지 확인 |
...
- 확정:
  ...
  - M3의 첫 source type은 CSV/local file로 시작한다.
  - M3의 metadata store는 SQLite로 시작하되, `MetadataStore` 경계를 둬 PostgreSQL 또는 MongoDB로 교체 가능하게 설계한다.
  - M3는 file upload UI가 아니라 repo/container 내부 sample path 등록으로 제한한다.
  - M4의 pipeline result storage는 local CSV result store로 시작한다.
- 열림:
  - M6 이후 첫 RDB source를 PostgreSQL로 할지 MySQL로 할지 결정해야 한다.
  - M7에서 row filter, drop, union, SQL transform 중 어디까지 먼저 구현할지 결정해야 한다.
