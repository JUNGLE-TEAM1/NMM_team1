...
- [ ] Product Health demo question CTA가 `/query` 화면에 표시된다.
- [ ] CTA 클릭이 기존 query 실행 흐름을 재사용한다.
- [ ] 미지원 질문 예시는 unsupported/blocked 표시를 확인할 수 있게 분리된다.
- [ ] 기존 query/evidence 표시가 깨지지 않는다.
