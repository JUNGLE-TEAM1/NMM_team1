...
- 2026-06-27: 기본 Python에는 `pandas`, 번들 Python에는 `pyarrow`가 없어 임시 runtime target `/tmp/asklake_pyarrow_runtime`에 `pyarrow`를 설치해 generated data만 생성했다. repo dependency에는 추가하지 않았다.
