# M2 Taxi 5GB local Spark evidence 사람 확인 게이트
...
- Human response: "짧게 local Spark 5GB 먼저 실행"으로 PySpark local 5GB evidence 범위를 승인함.
...
- Human response: Docker Spark cluster와 Airflow DAG 내부 Spark 호출은 제외하고, 기존 M2 evidence result shape를 유지하라는 이전 지시를 적용함.
...
- Status: local evidence recorded
- Ask human to confirm:
  - test/build/smoke 명령
  ...
  - completion criteria
- Human response: local Spark 5GB 실행 결과를 summary와 workspace 문서에 기록함. PR 전 추가 자동 검증은 AI가 수행한다.

## Quality Gate Confirm / 품질 게이트 확인

- Status: local gate recorded
- Ask human to confirm:
  - TDD 적용 또는 의도적 생략
  - 필요한 branch check와 CI gate
  - 생략한 검증과 이유
  - 관련 있는 deploy/publish gate
- Human response: PySpark focused test와 5GB local smoke를 branch 증거로 사용한다.
