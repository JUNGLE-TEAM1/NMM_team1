...
- 작은 fixture 또는 임시 데이터로 read/write smoke를 검증한다.
...
- TLC NYC Taxi Parquet은 실제 대용량 파일을 commit하지 않고 local/manual evidence 대상으로 둔다.
...
- [ ] `RuntimeConfig`가 Taxi 전용 필드 없이 입력 format/path/options를 표현한다.
...
- input format이 `parquet`인 작은 fixture read
...
- Amazon Reviews JSON transformation
...
M2 SparkRunner smoke를 구현한다. 작은 fixture를 읽고 Parquet output을 쓰며 `Week2RunnerResult` 호환 metrics를 반환한다. Taxi 전용 runner나 transformation semantics를 만들지 않는다.
```
...
- [ ] 작은 fixture로 read/write smoke가 통과한다.
- [ ] `status`, `row_count`, `bytes`, `duration_ms`, `output_path`가 반환된다.
- [ ] 대용량 Taxi 파일은 commit되지 않는다.
...
- [ ] 첫 PR 범위와 제외 범위가 report에 남아 있다.
- [ ] 후속 PR 후보가 next-actions에 남아 있다.
- [ ] local validation 결과가 quality에 남아 있다.
...
| PR | 목적 | 포함 | 제외 |
| --- | --- | --- | --- |
| PR 1 | `RuntimeConfig` + `SparkRunner` smoke | 작은 fixture read/write, 공통 result shape, basic tests | Taxi full ETL, M5 Catalog 연결, M6 SQL 연결 |
| PR 2 | TLC NYC Taxi evidence | local Taxi Parquet manual/benchmark evidence, row_count/bytes/duration/output_path 기록 | Taxi business Gold 지표, UI |
| PR 3 | M5 runner integration | M5가 optional SparkRunner를 선택/호출하고 성공 결과만 Catalog update 후보로 사용 | M3 transformation semantics |
| PR 4 | SQL smoke 연결 | output Parquet을 DuckDB 또는 Spark SQL로 count 확인 | M6 full AI Query/RAG 연결 |

PR 1 완료 뒤 팀 상황에 따라 PR 2와 PR 3의 순서는 조정할 수 있다.

## 완료 기준

- [ ] `RuntimeConfig` 최소 shape가 구현되어 있다.
- [ ] `SparkRunner` 또는 동등한 실행기 smoke가 작은 fixture로 read/write를 수행한다.
- [ ] `Week2RunnerResult` 호환 방향의 metrics가 반환된다.
- [ ] Taxi 전용 runner나 transformation semantics를 만들지 않는다.
- [ ] 대용량 원본/출력 데이터가 commit되지 않았다.
- [ ] TDD 상태 기록
- [ ] Acceptance 확인
