# Data integration flow skeleton 사람 확인 게이트
...
- Human response: 사용자가 "좋아 다음 페이즈 진행해"라고 지시해 B-1 범위로 진행.
...
- Human response: backend/API/schema 변경 없이 데이터 통합 화면 skeleton만 진행.
...
- Ask human to confirm:
  ...
  - completion criteria
- Human response: `npm run build`, `scripts/validate-harness.sh`, browser desktop/mobile DOM 확인 완료.
...
- Human response: TDD는 정적 UI skeleton 작업이라 생략, build/harness/browser verification으로 확인.
...
- Ask human to confirm:
  ...
  - 남은 위험
  - 다음 작업 문맥
- Human response: B-1 구현과 로컬 검증 완료. 다음 자연스러운 단계는 B-2 Source step.
