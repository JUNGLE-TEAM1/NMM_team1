...
- `gold_taxi_daily_metrics` schema, daily aggregation, valid row 조건은 Taxi Parquet 처리 가능성을 보여주기 위한 임시 evidence scaffold다. M2의 최종 책임은 runner/runtime/storage 실행과 row_count, bytes, duration, output path evidence이며, Gold metric 정의와 QualityRule은 M3가 소유한다.
- M3 `TransformSpec` / `QualityRule` 실행 경로가 붙으면 현재 하드코딩 Taxi Gold builder는 제거하거나 demo/test fixture로 격하한다.
...
- 이 최소 계약은 최종 M2 transform ownership이 아니라, M3 spec 실행 전까지 쓰는 Taxi evidence 기준이다.
