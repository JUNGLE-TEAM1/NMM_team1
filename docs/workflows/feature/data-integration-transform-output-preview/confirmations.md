# Data integration transform output preview 사람 확인 게이트

AI가 멈추고 사람 확인을 받아야 하는 지점을 기록한다.

## Scope Confirm / 범위 확인

- Status: completed by user instruction
- Ask human to confirm:
  - branch/workspace
  - 포함 범위
  - 제외 범위
  - 영향받는 Source of Truth 문서
- Human response: 사용자가 Transform 표현 보강을 진행하라고 지시함.
...
- Human response: backend/API/schema/payload 변경 없이 UI preview만 추가.
...
- Ask human to confirm:
  ...
  - completion criteria
- Human response: `npm run build`, `scripts/validate-harness.sh`, browser smoke로 확인.
...
- Human response: TDD는 생략하고 local build/harness/browser smoke를 적용.
...
- Human response: 자동 pull/merge/rebase 없이 branch 생성 후 local validation 진행.
...
- Ask human to confirm:
  ...
  - 남은 위험
  - 다음 작업 문맥
- Human response: 데모 URL에서 사람 확인 필요.
