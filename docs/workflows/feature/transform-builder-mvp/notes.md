...
- recommended template 저장 시 `steps[]` 원본과 별도 `builder_config`에 Source role column mapping과 review/locked 상태를 남기도록 했다.
- 데모 UX 조정: Target Dataset wizard의 `Process`를 처리 계획으로 정리하고, 추천 템플릿 기본 화면은 Source Dataset -> 내부 처리 -> Gold Target 자동 설정 완료 요약으로 두었다. 고급 설정은 내부 처리 단계 검토 요약으로 단순화했다.
...
- XFLOW처럼 처리 계획 화면을 단순화하되, multi-source 특성상 좌측은 단일 source table이 아니라 `reviews`, `behavior`, `delivery`, `product_master` role mapping으로 표현한다.
