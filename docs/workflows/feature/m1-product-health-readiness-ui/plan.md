...
- [ ] `dataset_product_health_gold` 준비/미준비 상태가 화면에 표시된다.
- [ ] Gold output 없음, Catalog 없음, local path 없음, query 준비됨 상태가 구분된다.
- [ ] 미준비 상태에서 다음 필요 작업이 M2/M3/M5 책임으로 표시된다.
- [ ] fake success 또는 임의 row 생성이 없다.
- [ ] `cd frontend && npm run build` 통과
- [ ] readiness keyword scan 통과
