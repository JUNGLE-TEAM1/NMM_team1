...
| `scripts/prepare-pr.sh` | `--close-issue` closes linked issue after merged stacked PR and records status | Stacked PR merge to a non-default branch may not auto-close issues | Low; explicit remote action only |
