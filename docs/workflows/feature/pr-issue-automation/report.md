...
- Changed: branch 생성 시 issue 기본 생성, 예외용 `--no-issue`, `scripts/prepare-pr.sh` PR handoff, sync/status/PR template/policy 문서 확장
- Verified: `bash -n`, `scripts/prepare-pr.sh --dry-run`, `scripts/status-workflow.sh`, `scripts/validate-harness.sh`, `scripts/validate-harness.sh --strict`
- Remaining: push/PR 생성은 사용자가 명시 플래그로 실행할 때 수행
- Next context: 다음 PR handoff 때 `scripts/prepare-pr.sh <workspace>` 사용
- Risk: GitHub CLI 미설치/미인증 환경에서는 issue/PR 원격 작업이 skip 또는 실패로 기록됨
