...
- Changed: branch 생성 시 issue 기본 생성, 예외용 `--no-issue`, branch switch 전 dirty checkpoint commit, 재발 방지 하네스 규칙 승인 절차, stacked PR merge 후 issue close 처리, `scripts/harness-flow-check.sh`, `scripts/prepare-pr.sh` PR handoff, sync/status/PR template/policy 문서 확장
- Verified: `bash -n`, `scripts/start-workflow.sh --dry-run`, `scripts/prepare-pr.sh --dry-run`, `scripts/prepare-pr.sh --close-issue`, `scripts/status-workflow.sh`, `scripts/validate-harness.sh`, `scripts/validate-harness.sh --strict`, `scripts/harness-flow-check.sh docs/workflows/feature/pr-issue-automation`
