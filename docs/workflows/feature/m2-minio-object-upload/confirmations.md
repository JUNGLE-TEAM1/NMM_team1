# M2 MinIO 실제 업로드 smoke 사람 확인 게이트
...
- Human response: user approved proceeding with the recommended M2 MinIO object upload slice. Scope is local MinIO object upload smoke; AWS S3, Spark cluster, M3 TransformSpec, M5 Airflow DAG internals, and M6 SQL are excluded.
...
- Human response: contract changes are limited to `RuntimeConfig.storage`, opt-in upload task behavior, `docs/03`, `docs/06`, and `contracts/runtime_config.sample.json`. Secret values must stay outside Git.
...
- Ask human to confirm:
  ...
  - completion criteria
- Human response: TDD/focused tests, workflow catalog regression, actual local MinIO smoke, full backend tests, and harness validation are the expected verification path.
...
- Human response: TDD applies. CI is required for the PR. No deploy/publish gate is required. AWS S3 and Spark cluster checks are intentionally skipped.
...
- Human response: local main was fast-forwarded to `origin/main` before workspace creation. Pre-PR sync will be checked before push/PR.
...
- Ask human to confirm:
  ...
  - 남은 위험
  - 다음 작업 문맥
- Human response: implementation summary, local MinIO smoke, test results, remaining risks, and next context are recorded in `report.md` and `quality.md`.
