...
- Changed: Taxi pyarrow runner now accepts a Parquet file or directory of Parquet files and records directory byte totals. Added `pyspark==4.1.2`, `Week2TaxiSparkRunner`, and `scripts/week2_m2_taxi_spark_local_evidence.py` for PySpark local mode Taxi evidence. Workspace plan/quality/next actions were updated.
- Verified: PySpark local install smoke succeeded with Spark `4.1.2`; focused Taxi pyarrow/Spark/storage tests passed; M5 boundary-focused suite passed; local January 2024 Spark demo smoke succeeded; temporary MinIO smoke succeeded with `spark_upload_taxi_daily_metrics`.
