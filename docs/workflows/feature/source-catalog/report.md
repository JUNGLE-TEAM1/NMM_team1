...
- Changed: SQLite-backed `MetadataStore`, CSV inspector, source/catalog API, sample CSV, frontend source registration and catalog detail, M3 smoke checks, source catalog docs/manual/regression notes, backend/frontend layer modularization
...
- Risk: SQLite is MVP-local. Store boundary and string UUIDs reduce future PostgreSQL/MongoDB migration risk, but no migration/import command is implemented yet. Current modularization separates first M3 layers but pipeline runner/result storage ports are still future M4 work.
