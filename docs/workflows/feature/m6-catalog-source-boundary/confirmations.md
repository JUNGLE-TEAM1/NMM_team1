...
- Human response: 공유 contract 변경 없이 내부 `CatalogSource` 경계만 추가.
...
- Ask human to confirm:
  ...
  - completion criteria
- Human response: focused/backend/compile/json/diff/harness 검증 완료.
...
- Human response: TDD 적용, local quality gate 통과. 원격 CI는 PR 전 확인 필요.
...
- Human response: start workflow 완료. 구현 후 pull/merge/rebase는 사람 확인 없이 실행하지 않음.
