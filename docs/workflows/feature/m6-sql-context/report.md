...
- Verified: direct service smoke; focused `backend/tests/test_week2_ai_query.py` passed; full `backend/tests` passed with temporary dependency target; `git diff --check`; `scripts/validate-harness.sh`; `scripts/validate-harness.sh --strict`; PR #193 remote checks passed.
