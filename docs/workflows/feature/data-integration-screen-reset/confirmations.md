# Data integration screen reset 사람 확인 게이트

AI가 멈추고 사람 확인을 받아야 하는 지점을 기록한다.

## Scope Confirm / 범위 확인

- Status: confirmed by user instruction
- Ask human to confirm:
  - branch/workspace
  - 포함 범위
  - 제외 범위
  - 영향받는 Source of Truth 문서
- Human response: 사용자가 "오케이 그러면 페이즈를 진행해줘"라고 지시해 Phase A 범위로 진행.

## Contract Confirm / 계약 확인

- Status: confirmed no contract change
- Ask human to confirm:
  - data model 변경
  - interface/API/CLI/UI contract 변경
  - external dependency
  - 공유 Source of Truth 변경
- Human response: backend/API/schema 변경 없이 데이터 통합 화면 reset만 진행.
...
- Ask human to confirm:
  ...
  - completion criteria
- Human response: `npm run build`, `scripts/validate-harness.sh`, browser DOM/screenshot 확인 완료.
...
- Human response: TDD는 UI reset 작업이라 생략, build/harness/browser verification으로 확인.
...
- Human response: branch 생성/전환은 수행했으나 pull/merge/rebase는 사람 확인 없이 실행하지 않음. 현재 branch는 원격보다 behind 13 상태라 PR-ready 전 sync 확인 필요.
...
- Ask human to confirm:
  ...
  - 남은 위험
  - 다음 작업 문맥
- Human response: Phase A 구현과 로컬 검증 완료. 다음 자연스러운 단계는 B-1 flow skeleton.
