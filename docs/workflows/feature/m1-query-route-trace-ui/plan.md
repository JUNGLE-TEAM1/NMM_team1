...
- [x] `/query` 화면이 `route=sql`을 표시한다.
- [x] `/query` 화면이 `route=unsupported` 또는 blocked 상태를 성공처럼 표시하지 않는다.
- [x] `retrieval_trace[]`가 있으면 source/score/matched terms/evidence index가 보인다.
- [x] `retrieval_trace[]`가 없어도 화면이 깨지지 않는다.
- [x] `cd frontend && npm run build` 통과
- [x] route/trace keyword scan 통과
