...
- NYC TLC Trip Record Data 중 M2 데모와 로컬 검증에 사용할 후보를 정한다.
  - 기본 후보: Yellow Taxi monthly Parquet
  - 권장 시작점: schema 변동 위험을 줄이기 위해 `yellow_tripdata_2024-01.parquet`를 로컬 기준 파일로 둔다.
  - 보류: `demo`, `fixed`의 정확한 row 수와 date range는 계약 mapping 후 확정한다.
- M2 Taxi Batch 데이터 범위 초안을 정한다.
  - `demo`: 화면과 smoke 확인용 작은 row 수
  - `fixed`: 리뷰/검산에 반복 사용 가능한 고정 범위
  - `local-full-month`: 2024년 1월 Yellow Taxi 파일 전체를 처리하는 로컬 기준 범위
  - `scale-target`: multi-month/year 또는 전체 Taxi dataset 적재를 검증하는 후속 확장 범위
...
NYC TLC Taxi 데이터 후보, demo/fixed/local-full-month/scale-target 범위, PostgreSQL table 초안, 기존 contracts에 대한 M2 mapping을 작성한다.
...
- [x] M2 Taxi 데이터 후보와 권장 시작 파일 또는 선택 기준이 기록되어 있다.
- [x] `demo`, `fixed`, `local-full-month`, `scale-target` 데이터 범위 초안이 기록되어 있다.
- [x] PostgreSQL 적재 대상 table, 주요 시간 컬럼, 주요 metric 컬럼 후보가 기록되어 있다.
- [x] `source_config`, `workflow_definition`, `execution_result`, `catalog_metadata`에 대한 M2 mapping이 기록되어 있다.
- [x] 대용량 원본/생성 데이터 commit 금지 기준이 기록되어 있다.
- [x] 실제 다운로드, 적재, connector, Parquet 생성, UI 구현은 후속 branch로 분리되어 있다.
- [x] M1/M5/M6와 맞춰야 할 열린 질문이 `notes.md`, `decisions.md`, 또는 `next-actions.md`에 기록되어 있다.
- [x] 문서 전용 작업으로 TDD 생략 사유가 `quality.md`에 기록되어 있다.
- [x] 필요한 정적 검증 또는 skip reason이 `quality.md`에 기록되어 있다.
- [x] `report.md`가 이번 branch의 변경, 검증, 남은 작업을 요약한다.
