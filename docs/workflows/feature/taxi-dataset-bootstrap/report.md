...
- Changed: M2 데이터 규모 전략을 `demo -> fixed -> local-full-month -> scale-target`으로 분리하고, `yellow_tripdata_2024-01.parquet`, `taxi_trips`, `gold_taxi_daily_metrics`, `demo=10,000 rows`, `fixed=pickup_date 2024-01-01` 기준 Taxi contract mapping 초안을 기록함.
- Verified: `scripts/validate-harness.sh`, `scripts/validate-harness.sh --strict` 통과
- Remaining: pre-merge sync 기록, Completion Confirm, PR 생성 여부 선택. PostgreSQL 적재 소유권과 MinIO prefix는 후속 구현/통합 branch에서 조율한다.
