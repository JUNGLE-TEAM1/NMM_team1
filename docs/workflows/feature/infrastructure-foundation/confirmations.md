# Infrastructure foundation 사람 확인 게이트
...
- Human response: CI/CD, Docker, Kubernetes, AWS approval gate 후보를 만들되 실제 AWS resource는 생성하지 않음.
...
- Ask human to confirm:
  ...
  - completion criteria
- Human response: shell syntax, Docker build, manifest shape, harness validation으로 확인.
...
- Human response: TDD는 제품 runtime logic 변경이 없어 미적용. 인프라 smoke/check 명령으로 검증.
...
- Human response: dirty workspace에서 Phase branch를 생성했으며 자동 pull/merge/rebase/push는 실행하지 않음.
...
- Ask human to confirm:
  ...
  - 남은 위험
  - 다음 작업 문맥
- Human response: 인프라 foundation 산출물과 검증 결과를 report에 기록.
