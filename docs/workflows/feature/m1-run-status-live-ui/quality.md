...
| Vite proxy run smoke | `curl -sS -X POST http://127.0.0.1:5173/api/week2/workflows/pipeline_reviews_json_e2e/runs ...` | passed | `ExecutionResult run_reviews_demo_003 fallback_succeeded ... 5 9` |
...
- Vite proxy smoke: `http://127.0.0.1:5173/api/week2/workflows/pipeline_reviews_json_e2e/runs`가 Week2 backend로 전달되는지 확인
...
- Vite proxy smoke: `ExecutionResult run_reviews_demo_003 fallback_succeeded s3://asklake-demo/reviews/gold/run_id=run_reviews_demo_003/ 5 9` returned
...
- dev proxy는 `localhost:8000` 대신 `127.0.0.1:8000`을 target으로 사용한다. 검수 중 Docker의 IPv6 `*:8000` listener가 `localhost:8000` 요청을 받아 Week2 route가 404가 되는 상황을 확인했다.
