# M5 Airflow smoke integration 사람 확인 게이트

AI가 멈추고 사람 확인을 받아야 하는 지점을 기록한다.

## Scope Confirm / 범위 확인

- Status: satisfied by user instruction
- Ask human to confirm:
  - branch/workspace
  - 포함 범위
  - 제외 범위
  - 영향받는 Source of Truth 문서
- Human response: 사용자가 추천안 구현을 지시했다. 범위는 local Airflow smoke 연결이며 production 완성은 제외한다.

## Contract Confirm / 계약 확인

- Status: satisfied by accepted decision
- Ask human to confirm:
  - data model 변경
  - interface/API/CLI/UI contract 변경
  - external dependency
  - 공유 Source of Truth 변경
- Human response: shared result JSON artifact 방식으로 adapter/DAG handoff를 구현한다.
...
- Status: satisfied by local evidence
- Ask human to confirm:
  - test/build/smoke 명령
  ...
  - completion criteria
- Human response: TDD, focused tests, full backend tests, DAG syntax, compose config, real local Airflow smoke, artifact/output/Catalog 검증 완료.
...
- Human response: production deploy, MinIO/S3, SparkRunner, async UI는 scope out으로 기록.
...
- Human response: branch created from `origin/main` at `8812690`; pull/merge/rebase는 사람 확인 없이 실행하지 않았다.
...
- Ask human to confirm:
  ...
  - 남은 위험
  - 다음 작업 문맥
- Human response: 변경/검증 결과를 final briefing에서 공유한다.
