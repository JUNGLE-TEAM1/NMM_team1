# Minimal pipeline run 사람 확인 게이트
...
- Human response: 사용자가 M4 시작과 이전 작업 마무리를 지시함. 범위는 M4 최소 pipeline run과 배포 smoke로 제한.
...
- Human response: pipeline API/UI contract, SQLite metadata persistence, local result store 추가를 M4 범위로 처리. 외부 dependency와 실제 AWS deploy는 추가하지 않음.
...
- Ask human to confirm:
  ...
  - completion criteria
- Human response: backend pytest, frontend build, compose smoke, Kubernetes manifest smoke, harness strict validation으로 검증.
...
- Human response: TDD 적용. PR CI는 PR 생성 후 확인 필요. deploy/publish는 이번 branch에서 실행하지 않음.
...
- Human response: start workflow가 base commit `e22bde2`에서 workspace/issue #35를 생성. 완료 전 pre-merge sync는 PR 준비 단계에서 수행.
...
- Ask human to confirm:
  ...
  - 남은 위험
  - 다음 작업 문맥
- Human response: M4 구현과 local 검증 완료. PR 생성 전 최종 sync/prepare-pr 단계가 남음.
