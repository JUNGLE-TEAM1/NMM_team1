# M6 answer evidence grounding 사람 확인 게이트
...
- Human response: 사용자가 "마지막 단계 개발 진행해"라고 지시했다. 범위는 M6 final evidence grounding으로 제한한다.
...
- Human response: `AIQueryResult.evidence`에 optional grounding fields를 추가한다. 기존 필드는 유지해서 M1 소비 호환성을 보존한다.
...
- Ask human to confirm:
  ...
  - completion criteria
- Human response: TDD 적용. focused M6 테스트, backend 테스트, compileall, contract JSON parse, harness validation으로 검증한다.
...
- Human response: core logic 및 interface fixture 보강이므로 TDD와 branch checks를 적용한다. deploy/publish gate는 해당 없음.
...
- Human response: `git fetch origin main` 후 `scripts/start-workflow.sh feature m6-answer-evidence-grounding "M6 answer evidence grounding"`로 시작했다. pull/merge/rebase는 실행하지 않았다.
...
- Ask human to confirm:
  ...
  - 남은 위험
  - 다음 작업 문맥
- Human response: 사용자 지시 범위 안에서 M6 final evidence grounding 구현, 검증, report 기록까지 완료한다. PR merge/finalize/cleanup은 PR 생성 뒤 별도 사람 확인이 필요하다.
