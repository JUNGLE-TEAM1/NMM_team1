# M2 Docker Spark Taxi evidence 사람 확인 게이트
...
- Human response: user supplied exact Docker driver/image/mount/smoke/worker/failure-scope decisions on 2026-06-29.
...
- Human response: result shape must stay compatible with existing M2 evidence / `Week2RunnerResult`; schema drift is runner correction target.
...
- Ask human to confirm:
  ...
  - completion criteria
- Human response: small Taxi smoke first, then 5GB Taxi smoke, both on Docker Spark cluster.
...
- Human response: Docker Spark cluster is required for this step; Airflow and Product Health 5GB are explicit follow-ups.

## Git Sync Confirm / Git sync 확인

- Status: confirmed-with-limitation
- Ask human to confirm:
  - 구현 전 start sync command/result
  - mid-phase upstream change action
  - 완료 전 pre-merge sync command/result
- Human response: user approved merge/proceed. GitHub issue/PR automation remains blocked by auth and is recorded in `sync.md`.
...
- Status: ready-with-auth-blocker
- Ask human to confirm:
  - 변경 요약
  - 검증 결과
  - 남은 위험
  - 다음 작업 문맥
- Human response: PR creation step may require GitHub auth repair.
