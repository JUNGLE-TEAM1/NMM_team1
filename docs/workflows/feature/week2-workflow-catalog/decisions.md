...
| Demo metrics fixture | `backend/samples/amazon_reviews_demo.jsonl` 10-row JSONL을 사용해 node별 transform 차이와 aggregate output 4 rows를 계산 | M3 fixed/extended sample 전에도 #92 `ExecutionResult` metrics wiring과 UI node board를 검증하기 위해 | #92 및 node board demo slice, 2026-06-25 |
