# M2 L6 preview runner adapter 사람 확인 게이트
...
- Human response: 사용자가 “그래 진행하자” 이후 “머지하고 다음꺼 진행해”라고 지시했다. 범위는 M2가 M3 L6 preview-only spec을 받아 작은 preview 실행과 `Week2RunnerResult` 호환 evidence를 만드는 것이다. Bronze/Silver/Gold 정책 생성, Docker Spark cluster, Airflow DAG 내부 호출, 5GB run은 제외한다.
...
- Human response: `RuntimeConfig.transform_spec`와 `RuntimeConfig.transform_spec_path`를 additive contract로 추가하고, `docs/03-interface-reference.md`와 `contracts/runtime_config.sample.json`에 반영한다. 외부 dependency는 추가하지 않는다.
...
- Ask human to confirm:
  ...
  - completion criteria
- Human response: focused test, contract sample smoke, backend full tests, JSON contract validation, compileall, diff check, harness validation을 수행한다.
...
- Human response: core runner contract 변경이므로 TDD를 적용한다. deploy/publish gate는 필요 없다.
...
- Human response: start-workflow가 `e1ddef2` 기준에서 workspace와 branch를 생성했다. 완료 전 main sync와 PR-ready 확인을 별도로 수행한다.
...
- Ask human to confirm:
  ...
  - 남은 위험
  - 다음 작업 문맥
- Human response: local validation completed. PR merge/finalize/cleanup은 PR 생성 후 별도 사람 확인을 받는다.
