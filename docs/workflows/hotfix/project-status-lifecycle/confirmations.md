# Project status lifecycle 사람 확인 게이트
...
- Human response: 사용자 요청 "프롬프트를 적용해줘"에 따라 Project status lifecycle hotfix 범위로 진행.
...
- Human response: GitHub Issue / PR / Project 운영 계약 변경으로 보고 `docs/11-git-sync-policy.md` 중심으로 반영.
...
- Ask human to confirm:
  ...
  - completion criteria
- Human response: shell syntax, `scripts/test-harness.sh`, `scripts/validate-harness.sh --strict`, `scripts/status-workflow.sh`로 검증.
...
- Human response: TDD 적용. PR open -> Review와 open PR + closed issue mismatch 회귀 테스트 추가.
...
- Human response: start sync는 workspace 생성 시 기록. pull/merge/rebase 없이 hotfix branch에서 진행. Pre-Merge는 PR 준비 전 local validation 기준으로 기록.
...
- Ask human to confirm:
  ...
  - 남은 위험
  - 다음 작업 문맥
- Human response: 완료 후 PR-ready 조건을 통과하면 자동 PR 생성까지 진행.
