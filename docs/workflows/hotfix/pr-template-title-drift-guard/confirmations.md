# PR 템플릿 제목 drift guard 보강 사람 확인 게이트
...
- Human response: 제품/API/schema/external dependency 변경 없음
...
- Ask human to confirm:
  ...
  - completion criteria
- Human response: local harness regression, strict validation, live read-only audit, status smoke 수행
...
- Human response: TDD applies yes; `scripts/test-harness.sh` fixture update and validation recorded in `quality.md`
...
- Human response: Start Sync recorded at `e11ff8b`; pull/merge/rebase 실행 없음; Pre-Merge Sync는 local validation 결과로 기록
...
- Ask human to confirm:
  ...
  - 남은 위험
  - 다음 작업 문맥
- Human response: 변경/검증/위험/다음 문맥을 `report.md`에 기록
