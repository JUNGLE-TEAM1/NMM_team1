...
- Failing test first: `scripts/test-harness.sh`에 영어-only PR 제목 drift, PR 번호 단순 참조 오탐 방지, record drift PR-ready 차단 fixture 추가
- Expected failure command/result: 구현 전에는 영어-only 제목 통과, `PR #105` 단순 참조 closing keyword 오탐, drift 상태의 `PR checklist ready: yes` 문제가 재현됨
...
| live read-only audit | `scripts/audit-github-records.sh --pr 123 --pr 124 --pr 126` | expected drift | `#123`만 drift, `#124` closing keyword 오탐 없음, `#126` 통과 |
