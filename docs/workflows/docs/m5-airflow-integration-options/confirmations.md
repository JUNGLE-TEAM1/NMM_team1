# M5 Airflow integration option guide 사람 확인 게이트
...
- Human response: "각 옵션에 대해서 어떤 선택지들이 있고, 각 선택지가 뭐를 의미하고 어떤 장단이 있는지, 어떤걸 추천하는지, 보통 어떤 경우에 어떤 것을 결정해야 하고 지금 프로젝트 상황에서는 어떤거를 선택해야 하는지를 설명하는 문서를 만들어주세요."

## Contract Confirm / 계약 확인

- Status: deferred to implementation slice
- Ask human to confirm:
  - data model 변경
  - interface/API/CLI/UI contract 변경
  - external dependency
  - 공유 Source of Truth 변경
- Human response: 이번 branch는 계약을 확정하지 않고 선택지를 문서화한다.
...
- Status: satisfied for docs-only
- Ask human to confirm:
  - test/build/smoke 명령
  ...
  - completion criteria
- Human response: code/test 변경 없이 문서와 현재 local 상태 확인만 수행한다.
...
- Human response: TDD/CI는 code behavior 변경이 없어 생략한다.
...
- Human response: start workflow는 자동 pull/merge/rebase 없이 workspace를 만들었다. `origin/main` fetch 후 현재 branch HEAD가 `origin/main`의 ancestor임을 확인했다.
...
- Ask human to confirm:
  ...
  - 남은 위험
  - 다음 작업 문맥
- Human response: final response에서 변경 요약과 남은 결정을 보고한다.
