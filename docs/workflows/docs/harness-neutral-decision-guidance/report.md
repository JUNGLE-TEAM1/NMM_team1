...
- Next context: 새 판단 질문 기준은 기존 온보딩 guide 위에 얹는 보강이다. scope 확장, merge, 검증 생략, interface 변경, 배포 질문에는 "AI가 괜찮다고 했다"가 아니라 기본 3축과 필요한 보조 축, 추천도/신뢰도/반대 관점/팀 영향/보완책/사람 확인 지점이 함께 있는지 확인한다. 질문이 `A가 없어서 B를 못 하겠어요. A부터 해야겠죠?` 형태라면 전제 검토 축으로 `B가 정말 A 전체에 의존하는가`를 먼저 확인한다.
