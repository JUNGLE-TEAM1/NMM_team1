...
- Changed: 팀원용 사용 가이드에 AI가 질문자의 원하는 결론을 맞춰주는 방식으로 답하지 않도록 하는 중립적 판단 기준을 추가했다. 판단 질문에는 문서 기준/현재 상태 구분, 반대 관점, 장점, 리스크, 보완책, 추천도, 신뢰도, 사람 확인 지점을 함께 제시하도록 했고, AI 답변을 최종 권위로 쓰지 않으며 제안자는 다른 팀원의 시간/스코프/리뷰/검증 부담과 감정적 비용을 함께 설명해야 함을 FAQ/checklist에 보강했다.
...
- Next context: 새 판단 질문 기준은 기존 온보딩 guide 위에 얹는 보강이다. scope 확장, merge, 검증 생략, interface 변경, 배포 질문에는 "AI가 괜찮다고 했다"가 아니라 추천도/신뢰도/반대 관점/팀 영향/보완책/사람 확인 지점이 함께 있는지 확인한다.
