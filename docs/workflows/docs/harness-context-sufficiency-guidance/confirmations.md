# Harness context sufficiency guidance 사람 확인 게이트
...
- Human response: 사용자가 "프롬프트를 반영해줘"라고 요청해 전제/맥락 확인 규칙 보강 범위를 승인한 것으로 기록. 범위는 협업 하네스 문서와 report 보강으로 제한.
...
- Human response: API/data/interface/schema contract 변경 없음. 협업 문서 규칙만 변경.
...
- Ask human to confirm:
  ...
  - completion criteria
- Human response: 문서 보강 완료 기준에 따라 `rg`, `git diff --check`, `scripts/validate-harness.sh`, `scripts/test-harness.sh`로 검증.
...
- Human response: 문서 규칙 변경이라 TDD는 적용하지 않고 하네스 validation/test를 품질 게이트로 사용.
...
- Human response: 현재 branch가 `origin/main`보다 behind 13이나 pull/merge/rebase는 사람 확인 없이 실행하지 않음. PR 전 sync 판단 필요.
...
- Ask human to confirm:
  ...
  - 남은 위험
  - 다음 작업 문맥
- Human response: 변경 요약, 검증 결과, 남은 sync risk를 report와 final response에 기록.
