# WSL2 known gaps guidance 사람 확인 게이트
...
- Human response: 사용자가 보완 프롬프트 적용을 요청함. 문서 보강만 진행한다.
...
- Human response: interface/API/schema 변경 없음. Source of Truth 문서 보강은 `docs/04`와 manual verification에 제한한다.
...
- Ask human to confirm:
  ...
  - completion criteria
- Human response: `scripts/validate-harness.sh`, `scripts/validate-harness.sh --strict`, `git diff --check`를 실행한다.
...
- Human response: docs-only 변경이라 TDD는 미적용하고 local validation으로 확인한다.
...
- Human response: branch created from `origin/main` at `3d5192b`; pull/merge/rebase는 실행하지 않았다.
...
- Ask human to confirm:
  ...
  - 남은 위험
  - 다음 작업 문맥
- Human response: 완료 판단은 local validation 결과와 report 기록으로 한다. PR/push는 별도 사람 확인 전 실행하지 않는다.
