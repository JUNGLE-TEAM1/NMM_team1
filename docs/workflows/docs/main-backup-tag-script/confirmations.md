# Main backup tag script 사람 확인 게이트
...
- Human response: 사용자가 project-context 프롬프트만으로는 다른 사람도 동일하게 실행하기 어렵다는 점을 확인하고, 공용 스크립트 추가와 merge까지 지시했다.
...
- Human response: CLI/API/schema contract 변경은 없고, `scripts/create-main-backup-tag.sh` 실행 스크립트만 추가한다.
...
- Ask human to confirm:
  ...
  - completion criteria
- Human response: dry-run smoke, shell syntax check, harness validation으로 검증한다.
...
- Human response: script/docs-only 변경이므로 app runtime build는 생략하고, `bash -n`, dry-run smoke, `git diff --check`, harness validation을 수행한다.
...
- Human response: 별도 worktree를 `origin/main` 기준으로 만들었고, pre-merge validation 통과 후 PR/merge까지 진행한다.
...
- Ask human to confirm:
  ...
  - 남은 위험
  - 다음 작업 문맥
- Human response: 사용자가 "새로 추가하고 머지까지해"라고 지시했다.
