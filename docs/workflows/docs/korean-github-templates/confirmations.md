# 한국어 GitHub Issue/PR 템플릿 개선 사람 확인 게이트
...
- Human response: 사용자가 GitHub Issue/PR 템플릿 개선 반영과 PR 진행을 지시함. 범위는 `.github` 템플릿과 대응 branch workspace 문서로 제한.
...
- Human response: API/schema/data model/external dependency 변경 없음. Source of Truth 규칙 문서는 변경하지 않음.
...
- Ask human to confirm:
  ...
  - completion criteria
- Human response: Markdown/YAML 템플릿 변경이므로 `git diff --check`, Issue template YAML parse, harness validation, PR sync preflight로 검증.
...
- Human response: TDD는 런타임 로직이 없어 적용하지 않음. local validation과 PR CI/check 확인으로 품질 게이트를 운영.
...
- Human response: `main` 기준 worktree에서 시작했고 pull/merge/rebase는 실행하지 않음. pre-PR sync 결과를 `sync.md`에 기록.
...
- Ask human to confirm:
  ...
  - 남은 위험
  - 다음 작업 문맥
- Human response: 사용자가 PR 진행을 지시함. merge/finalize/cleanup은 PR 생성 후 별도 사람 확인 전 실행하지 않음.
