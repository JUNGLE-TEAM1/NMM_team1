...
- Changed: Source of Truth Impact Gate docs, status display, strict validation guard, CI checkout depth for harness validation, workspace evidence
...
- Risk: strict guard depends on base commit availability in CI; harness workflow now uses full fetch depth to avoid shallow checkout false positives
