# Big dataset manipulation context alignment 사람 확인 게이트
...
- Human response: 사용자가 대용량/복합 데이터셋 조작 목표를 프로젝트 문맥에 반영하라고 요청함.
...
- Human response: 새 API/schema/runtime contract는 추가하지 않고 기존 Source of Truth 문구와 evidence 의미를 보강한다.
...
- Ask human to confirm:
  ...
  - completion criteria
- Human response: 문서 검색, `scripts/validate-harness.sh`, `scripts/validate-harness.sh --strict`로 검증한다. runtime test/build는 문서 전용 변경이라 생략한다.
...
- Human response: `git fetch origin main` 후 최신 `origin/main`에서 `codex/big-dataset-context` 브랜치를 생성했다. pull/merge/rebase는 실행하지 않았다.
...
- Ask human to confirm:
  ...
  - 남은 위험
  - 다음 작업 문맥
- Human response: 대용량/복합 데이터셋 조작·가공 문맥 보강 완료. push/PR/merge는 사람 확인 전 실행하지 않음.
...
- Ask human to confirm:
  ...
  - CI 확인
  - merge/finalize 범위
- Human response: 사용자가 “pr 마무리해”라고 지시함. 원격 push와 PR 생성까지 승인된 것으로 해석하고, merge는 PR 상태와 추가 확인 후 진행한다.
