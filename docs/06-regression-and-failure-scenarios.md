...
| Failure condition | 독립 `M5 데모` 메뉴나 학습 화면을 다시 주 경로로 노출하거나, 저장 전 draft를 실행할 수 있게 하거나, Week2 fixture output을 새 Target Dataset 산출물처럼 표시한다. |
| Expected behavior | `/api/target-datasets/{dataset_id}/runs`는 저장된 Target Dataset draft만 실행 handoff 대상으로 받고, 화면은 `Job Runs`에서 run id/status와 fixture-backed smoke 범위를 표시한다. |
...
| Expected behavior | M2 storage adapter가 S3-compatible URI와 local fallback path를 함께 계산하고, opt-in upload smoke에서는 같은 object key로 MinIO/S3-compatible endpoint에 PUT한다. direct S3A smoke에서는 Spark writer가 `s3a://` output prefix에 직접 Parquet directory를 쓴다. 어떤 경로에서도 secret 값을 로그나 Git에 남기지 않는다. |
| Verification method | `backend/tests/test_week2_storage_adapter.py`, `backend/tests/test_week2_spark_runner.py`, `backend/tests/test_week2_workflow_catalog.py`, `backend/tests/test_week2_taxi_spark_s3a_config.py`를 확인한다. |
...
| 항목 | 내용 |
| --- | --- |
| Must not break | `DuckDBSqlEngine`은 `SqlEngineAdapter` 뒤에서만 사용하고, `CatalogMetadata.storage.local_fallback_path` 또는 local `storage_uri`를 기준으로 Parquet/JSONL을 읽어야 한다. |
| Failure condition | M6가 DuckDB를 직접 import하거나, CatalogMetadata가 준 path 대신 output file path를 추측하거나, SELECT-only/table allowlist/column allowlist/LIMIT/local path 확인 없이 SQL을 실행한다. |
...
### External Connection secret 또는 schema preview가 잘못 처리되는 경우
