...
- Week 2 workflow run request `executor` accepts `local_runner`, `airflow`, `spark_runner`, or `kafka_replay`. `spark_runner` means M5 calls the M2 `Week2SparkRunner` boundary directly; it does not mean Airflow DAG-internal Spark execution is already implemented. `kafka_replay` means M5 calls the M4 Kafka Replay API and records replay evidence/Catalog metadata; it does not make Kafka the main Gold ETL path.
- `KafkaTopicContract` is evidence and raw-event handoff for Week 2. Kafka is not a blocker for the Amazon Reviews main E2E path unless a later Phase explicitly changes the main path. The executable M4/M5 handoff uses `POST http://localhost:5189/api/replay/start` and stores `dataset_kafka_replay_raw_events` evidence under `data/week2/kafka/raw_events/run_id=<run_id>/`.
