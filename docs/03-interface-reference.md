...
| `POST` | `/api/target-datasets/{dataset_id}/runs` | `TargetDatasetRunCreate -> TargetDatasetRun` | 저장된 `job_definition` context를 Week 2 run API로 handoff한다. Product Health draft는 후속 실행/카탈로그 PR이 공유할 `product_health_manual_run_contract`도 함께 반환한다 |
...
C-4 runtime output은 아직 동적 Target Dataset 산출물이 아니라 Week2 fixture-backed output이다. 따라서 `target_dataset_handoff.runtime_output_scope=week2_fixture_output`, `runtime_output_dataset_id`, `runtime_pipeline_id`를 함께 내려 API evidence가 현재 실행 범위를 보존한다.
Product Health `process_rule.type=product_health_gold_pipeline`인 run은 `execution_result.product_health_manual_run_contract`를 추가로 포함한다. PR 5A의 이 block은 실제 Gold 실행이 아니라 PR 4/5B/6/7/8 연결 계약이다. 현재 상태값은 `pending_product_health_execution`이고 PR 5B가 source snapshot, Gold output, quality result, lineage, Catalog payload의 실제 값을 채운다. `schema_version`, Gold contract version, `allowed_columns`는 현재 `contracts/product_health_*.sample.json` 내용을 따른다. #310 같은 Gold v2 계약 PR이 머지되면 이 block도 해당 contract file의 v2 값을 기준으로 생성한다.
...
```json
{
  ...
  "status": "pending_product_health_execution",
  ...
  "gold_output": {
    ...
    "contract_version": "product_health_gold_contract_v1",
    "schema": [],
    "status": "pending_product_health_execution"
  },
  "quality_results": [
  ...
  "catalog_payload": {
    ...
    "storage_uri": null,
    "format": "parquet",
    "schema": {"schema_version": "schema_product_health_gold_v1", "fields": []},
...
PR 4 Source Snapshot은 `source_dataset_id` 기준으로 latest successful snapshot을 찾을 수 있어야 하며 snapshot artifact는 `format=parquet` 경로를 제공한다. PR 6 Catalog 등록은 `catalog_payload.storage_uri`, `schema`, `metrics`, `lineage`, `query.allowed_columns`, `m3_contract_refs`를 실제 값으로 채운 성공 run만 등록 대상으로 삼는다. `password`, connection string, raw secret은 이 계약 어디에도 포함하지 않는다.
