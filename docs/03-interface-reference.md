...
- Product Health recommended template draft: `{type: "product_health_recommended_template", mode: "recommended_template", template_id, template_version, target_dataset, query_table, source_mappings[], flow[], steps[], quality_rules[], output_schema[], metric_definitions[], locked_fields[], contract_claims}`.

Product Health recommended template draft는 M3 계약과 source role mapping을 metadata로 저장해 사용자가 review할 수 있게 하는 첫 조각이다. 현재 Target Dataset API는 multi-source metadata 저장까지만 닫으며, multi-source 실제 실행은 후속 PR 범위다.
