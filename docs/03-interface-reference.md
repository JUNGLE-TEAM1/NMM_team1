...
Product Health Processing Template slice는 M3 Product Health `TransformSpec` / Gold Contract를 Target Dataset wizard가 검토하고 저장할 수 있는 UI-oriented template으로 노출한다.
...
| `GET` | `/api/target-datasets/{dataset_id}/runs` | `TargetDatasetRun[]` | Target Dataset에 연결된 Job Runs 조회 |
...
- Product Health recommended template draft: `{type: "product_health_recommended_template", mode: "recommended_template", template_id, template_version, target_dataset, query_table, flow[], steps[], quality_rules[], output_schema[], metric_definitions[], locked_fields[], contract_claims}`.

Product Health recommended template draft는 M3 계약을 metadata로 저장해 사용자가 review할 수 있게 하는 첫 조각이다. 현재 Target Dataset API는 여전히 단일 `source_dataset_id`를 받으며, `reviews`, `behavior`, `delivery`, `product_master` role mapping과 multi-source 실행은 후속 PR 범위다.
`TargetDatasetRun.execution_result.target_dataset_handoff`는 `target_dataset_id`, `target_dataset_name`, `job_definition_status`, `source_dataset_id`, `selected_fields`, `process_rule`, `schedule`, `output_schema`를 포함한다.
...
- `GET /api/processing-templates/product-health`는 M1 Target Dataset Processing UI가 M3 Product Health `TransformSpec`을 추천 draft rule로 검토/저장하기 위한 조회 경계다. 이 endpoint는 `dataset_product_health_gold` / `gold_product_health`의 step list와 quality rules를 반환하지만, M2 execution, M5 catalog persistence, M6 SQL query 가능 상태를 주장하지 않는다.
