...
| `contracts/execution_result.sample.json` | M2, M3, M4, M5 | M1, M5, M6 | Airflow and local runner compatible execution result shape for the M5 workflow API |
...
- Week 2 workflow run request `executor` accepts only `local_runner` or `airflow`. Spark is not a direct M5 API executor; Spark work should be invoked inside the Airflow DAG task path or a later explicitly orchestrated integration.
