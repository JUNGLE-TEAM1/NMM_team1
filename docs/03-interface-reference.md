...
- M2 Taxi local batch evidence uses `pipeline_taxi_daily_metrics`, `dataset_taxi_daily_metrics_gold`, and `gold_taxi_daily_metrics`. It is supporting local Parquet evidence only, not the product risk representative path. PySpark local mode can read a single Parquet file or a Parquet directory and records input rows, input bytes, duration, output path, output rows, and output bytes. Docker Spark cluster execution, durable MinIO/S3 write, PostgreSQL loader, and Airflow DAG-internal invocation are later phases.
