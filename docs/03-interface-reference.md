...
| `POST` | `/api/target-datasets/{dataset_id}/runs` | `TargetDatasetRunCreate -> TargetDatasetRun` | 저장된 `job_definition`을 Week 2 `ExecutionResult` 생성으로 handoff |
...
`TargetDatasetRun.execution_result.target_dataset_handoff`는 `target_dataset_id`, `target_dataset_name`, `job_definition_status`, `source_dataset_id`, `selected_fields`를 포함한다.
...
| `TargetDatasetRun` | Dataset UX / Job | `id`, `target_dataset_id`, `week2_run_id`, `pipeline_id`, `executor`, `status`, `execution_result` | C-4는 M5 run handoff와 status 표시만 담당하고 runtime evidence 정렬은 C-5로 넘김 |
