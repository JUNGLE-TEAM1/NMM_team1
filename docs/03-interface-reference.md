...
- M5 Airflow local smoke uses a shared result artifact. `Week2AirflowAdapter` chooses the DAG by `pipeline_id`: reviews uses `ASKLAKE_WEEK2_AIRFLOW_DAG_ID` default `asklake_week2_reviews`, and product-health uses `ASKLAKE_WEEK2_PRODUCT_HEALTH_AIRFLOW_DAG_ID` default `asklake_week2_product_health`. Each DAG writes `data/week2/_airflow_results/<run_id>.json`; the artifact contains `week2_result` with the `Week2RunnerResult`-compatible fields `status`, `task_results`, `logs`, `row_count`, `bytes`, `duration_ms`, `output_path`, `output_row_count`, and `output_bytes`. `output_path` may be omitted when `catalog_payload.storage_uri` is present.
- Manual Run 또는 Airflow result가 `week2_result.catalog_payload`를 제공하면 M5는 이 payload를 Catalog 등록 입력으로 우선 사용한다. 최소 필드는 `dataset_id`, `name`, `layer`, `query_table`, `storage_uri`, `format`, `schema`, `row_count`, `quality_summary`, `lineage`, `m3_contract_refs`다. 이 경우 `catalog_payload.storage_uri`가 canonical output location이며, M5는 `run_id`나 `output_path`로 gold parquet 위치를 다시 추측하지 않는다.
- `CatalogMetadata.storage_uri`는 Manual Run `catalog_payload.storage_uri`를 보존한다. 현재 소비자 호환을 위해 `CatalogMetadata.s3_uri`도 같은 값을 mirror할 수 있다. `storage.local_fallback_path`는 `storage_uri`가 local path 또는 `file://`일 때만 채우고, S3-compatible object URI는 `storage.uri`/`storage_uri`에 보존한다.
