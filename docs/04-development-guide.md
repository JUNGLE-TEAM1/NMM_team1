...
시스템이 강제해야 하는 repository, CI, PR, issue/project, secret, review, deploy 안전장치는 `docs/system-guardrails.md`에 인벤토리로 둔다.
...
- 강제 가능한 안전장치는 GitHub/CI/platform에 두고, 하네스 문서는 작업 흐름과 복구 증거를 기록한다.
...
linked issue와 GitHub Project Status lifecycle은 `docs/11-git-sync-policy.md`를 따른다.
요약하면 branch start는 `In Progress`, PR open은 `Review`, PR merge/finalize는 issue close와 `Done`이다.
`scripts/start-workflow.sh`가 생성하는 GitHub issue는 한국어 제목 prefix, 한국어 body section, 작업 type별 label, `--body-file` 기반 본문 전달을 사용한다.
사람이 넘긴 제목 자체는 임의 번역하지 않고 스크립트가 붙이는 prefix와 기본 문구만 한국어화한다.

## 3) Git sync 규칙

branch 최신성, 통합 안전성의 Source of Truth는 `docs/11-git-sync-policy.md`다.
실제 system enforcement 후보는 `docs/system-guardrails.md`를 따른다.
...
issue/PR/Project lifecycle은 책임을 나눈다.
branch start issue 생성은 local branch creation을 GitHub가 직접 감지할 수 없으므로 `scripts/start-workflow.sh`가 담당한다.
PR linked issue 필수, PR closing keyword, Project status sync, lifecycle drift detection은 GitHub Actions, GitHub Project automation, required check 후보로 `docs/system-guardrails.md`에 기록한다.
...
- [ ] If there is no linked issue, the PR body includes an approved no-issue exception such as `No Linked Issue Exception: approved` or `연결된 Issue 예외: 승인` with a reason.
- [ ] The PR body uses the readable seven-section handoff shape: summary, narrative change explanation, validation, impact scope, reviewer focus, remaining/excluded work, and pre-merge checks.
- [ ] If the branch has a linked GitHub issue, `sync.md` records the GitHub Project lifecycle result from `docs/11-git-sync-policy.md`; open PRs use Project `Review`, and `Done` is only for merge/finalize-complete linked issues.
- [ ] If the branch issue was created by `scripts/start-workflow.sh`, generated issue title/body/labels follow the Korean issue template structure or a reason is recorded
...
- 하네스 job: fast path에서는 `scripts/validate-harness.sh`를 항상 실행하고, harness/script/CI/workspace evidence path 변경 때 `scripts/validate-harness.sh --strict`와 `scripts/test-harness.sh`를 조건부 실행한다.
