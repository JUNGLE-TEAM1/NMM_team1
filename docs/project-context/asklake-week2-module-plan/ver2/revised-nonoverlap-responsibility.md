...
- 발표용 범위는 Amazon Reviews JSON main path를 우선하고, Taxi/Kafka는 최소 contract/evidence로 제한한다.
...
M3가 모든 데이터 ETL을 맡으면 병목이 될 수 있다. 발표 전 우선순위는 Amazon Reviews JSON main path다. Taxi와 Kafka는 최소 source profile, raw event contract, metrics/evidence를 먼저 맞추고, 완전한 Silver/Gold 확대는 후속 slice로 둔다.
...
- `CatalogMetadata` retrieval과 dataset selection evidence를 만든다.
...
| `AIQueryResult` | M6 | M5 catalog, M3 facts | M1 | question, selected_datasets, evidence, SQL, rows, summary, chart_spec. |
...
| M3 | 기존 source/catalog 기반은 있으나 Week2 JSON 핵심 구현은 미병합/재조정 필요. | 범위 확대 위험이 있다. Amazon Reviews main path 우선으로 줄여야 한다. |
...
| M6 | AI query skeleton이며 real SQL/RAG는 후속 연결 필요. | 충돌 낮음. M5 Catalog와 M2 SQL runtime을 소비하도록 연결하면 된다. |
...
4. M3가 Amazon Reviews JSON main path profile/schema/ETL spec을 확정한다.
...
8. M6가 CatalogMetadata retrieval, SQL planner/guardrail, `AIQueryResult`를 반환한다.
