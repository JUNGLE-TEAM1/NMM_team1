...
- AI Query/분석 대표 경로는 Amazon Reviews JSON으로 우선 고정한다.
- Taxi/Kafka는 optional이 아니라 필수 처리/evidence 경로이며, M6 분석 연결은 이번 기본 범위가 아니라 후속 확장으로 둔다.
...
| --- | --- | --- | --- |
| Amazon Reviews JSON / JSONL | M3, M5, M6, M1 | profile/schema/TransformSpec, M5 Workflow/Catalog, M6 AI Query/Evidence, M1 표시까지 연결 | real LLM, 외부 vector DB |
