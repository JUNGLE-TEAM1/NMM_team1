...
- 외부 vector DB, full document RAG, 대규모 indexing, real LLM provider 연결을 이번 기본 범위로 소유하지 않는다.
...
| M6 | AI query skeleton이며 CatalogMetadata 기반 retrieval/RAG-lite는 기본 연결 대상이다. 외부 vector DB/full document RAG/real LLM은 후속 연결이다. | 충돌 낮음. M5 Catalog와 M2 SQL runtime을 소비하도록 연결하면 된다. |
...
8. M6가 CatalogMetadata semantic retrieval, RAG-lite evidence grounding, SQL planner/guardrail, `AIQueryResult`를 반환한다.
