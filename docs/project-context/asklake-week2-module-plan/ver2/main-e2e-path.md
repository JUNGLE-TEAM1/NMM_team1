...
-> M5 Catalog
-> M6 Semantic retrieval / RAG-lite evidence / AI Query
-> M1 UI
```
...
| M6 | M5 Catalog를 기반으로 CatalogMetadata retrieval, semantic dataset selection, RAG-lite evidence grounding, SQL planner/guardrail, `AIQueryResult`를 응답 경로에 연결한다. | fixture-only 설명이 아니라 Catalog metadata와 evidence를 근거로 답한다. |
...
- real LLM, 외부 vector DB, full document RAG, 대규모 indexing, Iceberg는 이번 분석 대표 path에 포함하지 않는다.
