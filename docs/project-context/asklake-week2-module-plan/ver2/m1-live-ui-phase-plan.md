...
- 기본 pipeline id는 `pipeline_reviews_json_e2e`를 소비할 수 있다.
- 기본 dataset id는 `dataset_reviews_gold`를 소비할 수 있다.
...
- 성공 run의 output dataset을 `/catalog/dataset_reviews_gold`로 연결한다.
...
- `rg -n "CatalogMetadata|dataset_reviews_gold|lineage|local_fallback_path" frontend/src`
- `scripts/validate-harness.sh --strict`
- `git diff --check`
...
- M6 PR #145가 merge되면 scoring 결과를 소비하되, M1에서 scoring을 재구현하지 않는다.
...
- M6 response shape가 PR #145 이후 조금 바뀔 수 있으므로 optional guard가 필요하다.
