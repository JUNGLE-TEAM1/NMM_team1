...
| PR linked issue required | GitHub Action required check | planned | PR merge blocked when linked issue or closing keyword is missing | maintainer | PR 단계에서 hard gate로 두는 것이 branch 시작 강제보다 현실적이다. |
...
- PR linked issue required check를 warning으로 시작할지 hard gate로 시작할지 결정한다.
