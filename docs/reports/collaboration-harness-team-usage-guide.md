...
1. 일은 한 번에 하나만 시킨다.
2. AI에게 "어디까지 할지" 말한다.
3. 사람은 자기 스코프를 이해한 뒤 AI에게 작업을 지시한다.
4. AI가 만들고, 고치고, 기록하고, 검증한다.
5. 작업 문맥은 AI가 초안을 만들고, 사람이 충분성을 확인한다.
6. PR을 올리는 것과 merge하는 것은 다르다.
7. merge, 배포, 비용, secret, DB 변경은 사람이 확인해야 한다.
8. AI 설명이 애매하면 다시 묻는다.
9. 다음 사람이 오해 없이 이어받을 수 있어야 문맥 저장이 완료된다.
10. 하네스는 나중에 크게 터질 문제를 앞에서 작게 잡아준다.
```
...
좋은 하네스 사용자는 모든 명령어를 외우는 사람이 아닙니다. 좋은 사용자는 자기 스코프를 이해하고, 중요한 선택 앞에서 무엇을 확인해야 하는지 알고, 이해가 안 되면 다시 묻는 사람입니다.
