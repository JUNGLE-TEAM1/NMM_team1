...
- Remaining: PR #149 main sync conflict 해결, 원격 CI/PR 검증, real SQL runtime adapter, M3 schema/profile facts 추가 반영
...
- Risk: workflow run 전 AI query는 기존 fixture fallback을 계속 사용한다. 발표 흐름에서 반드시 workflow run 후 query를 사용해야 최신 M5 catalog evidence가 보장된다. PR #149는 `docs/reports/README.md` report index 충돌 때문에 main sync 확인이 필요하다.
