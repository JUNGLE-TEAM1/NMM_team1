...
| Product Health Manual Run execution | [`product-health-manual-run-execution.md`](product-health-manual-run-execution.md) | Product Health Target Dataset run이 `source_snapshots[]` parquet artifact를 받아 `gold_product_health.parquet`와 Catalog handoff 계약을 실제 값으로 채우는지 확인 |
...
| AI Query Catalog 연결 | [`ai-query-dataset-context.md`](ai-query-dataset-context.md) | M6 AI Query가 Product Health CatalogMetadata의 `gold_product_health` table/path/query contract를 읽어 DuckDB SQL을 실행하는지 확인 |
...
| M6 OpenAI LLM Adapter | [`m6-openai-llm-adapter.md`](m6-openai-llm-adapter.md) | M6 Step 9에서 env-gated OpenAI provider adapter, secret-safe request body, provider fallback behavior를 확인 |
| M6 Answer UX Metadata | [`m6-answer-ux-metadata.md`](m6-answer-ux-metadata.md) | M6 Step 10에서 `answer_metadata`와 M1 `/query` answer generation panel로 provider/fallback/grounding state 표시를 확인 |
| M6 Week2 함수 흐름 공부 노트 | [`m6-week2-function-flow-study.md`](m6-week2-function-flow-study.md) | M6 1~10단계가 최종 `Week2AIQueryService.answer()` 함수 흐름 안에서 어떻게 연결되는지 학습용으로 확인 |
...
| Dataset Module Source Dataset persistence | [`source-dataset-persistence.md`](source-dataset-persistence.md) | C-2에서 External Connection 기반 Source Dataset metadata 저장/조회와 Target source picker 반영을 확인 |
