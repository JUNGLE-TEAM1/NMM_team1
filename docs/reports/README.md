...
| Source Snapshot large data readiness | [`source-snapshot-large-data-readiness.md`](source-snapshot-large-data-readiness.md) | Source Snapshot이 bounded sample evidence이며 full 5GB ingest/processed input evidence와 구분되는지 확인 |
| Product Health Source Inventory Binding | [`product-health-source-inventory-binding.md`](product-health-source-inventory-binding.md) | Product Health behavior/reviews/product catalog/delivery 원천이 raw/prepared/missing 상태로 Source Dataset 후보에 묶이는지 확인 |
| Product Health Silver Gold Run Execution | [`product-health-silver-gold-run-execution.md`](product-health-silver-gold-run-execution.md) | Product Health Gold Run이 prepared Gold reference 또는 Silver parquet materialization evidence를 남기고 C-39 Catalog publish 입력이 되는지 확인 |
| Catalog AI Query Clean-room Handoff | [`catalog-ai-query-clean-room-handoff.md`](catalog-ai-query-clean-room-handoff.md) | Product Health Gold Run publish 뒤 CatalogDataset과 AI Query selected/evidence/retrieval/path가 같은 run 결과를 가리키는지 확인 |
| Full Browser Demo Smoke | [`full-browser-demo-smoke.md`](full-browser-demo-smoke.md) | Product Health Source/Silver/Gold/Run/Catalog/AI Query 전체 브라우저 흐름과 남은 Hotfix 후보 확인 |
| Product Health Preset Synthesis | [`product-health-preset-synthesis.md`](product-health-preset-synthesis.md) | 사이트에서 Product Health preset 합성을 실행해 seed mapping, Silver/Gold parquet, Catalog/Evidence 준비 파일을 재생성하는지 확인 |
| Product Health External Source Contract | [`product-health-external-source-contract.md`](product-health-external-source-contract.md) | Product Health source inventory를 Kafka/PostgreSQL/MongoDB/S3 runtime source와 demo fallback evidence로 분리했는지 확인 |
| Product Health Runtime Connection Seed | [`product-health-runtime-connection-seed.md`](product-health-runtime-connection-seed.md) | Product Health용 Kafka/PostgreSQL/MongoDB/S3 External Connection metadata seed와 secret boundary 확인 |
| Product Health Source Save Alignment | [`product-health-source-save-alignment.md`](product-health-source-save-alignment.md) | Product Health Source Dataset 저장이 runtime scope를 primary로 유지하고 local/prepared artifact를 fallback evidence로 분리하는지 확인 |
| Product Health Runtime Seed Loaders | [`product-health-runtime-seed-loaders.md`](product-health-runtime-seed-loaders.md) | 11GB 합성 데이터셋 split을 Kafka/PostgreSQL/MongoDB/MinIO에 적재하는 operator loader와 dry-run evidence 확인 |
