...
- Changed: M1 `/query` 화면에 `AIQueryResult.route`와 `retrieval_trace[]` 표시를 추가했다.
...
- Browser unsupported route: `route=unsupported`, `blocked`, failure text, SQL 성공으로 처리하지 않는 warning 표시.
