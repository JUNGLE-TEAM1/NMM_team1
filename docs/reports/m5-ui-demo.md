...
- Verified: `npm run build` in `frontend/` -> passed; `POST http://127.0.0.1:5176/api/week2/workflows/pipeline_reviews_json_e2e/runs` -> `ExecutionResult` payload returned; backend `/api/health` -> ok; catalog API -> `CatalogMetadata.metrics.row_count=3`, `bytes=195`.
...
Expected ExecutionResult.row_count: 4
Expected CatalogMetadata.metrics.row_count: 3
