...
| C-48 | `feature/main-sync-product-health-runtime-stack` | `origin/main`의 AI Query route/RAG/LLM 계약을 현재 Product Health runtime/catalog 흐름 위에 단계적으로 이식하는 설계를 고정한다 | C-46 또는 현재 브랜치 명시 확인 | `AIQueryResult`, `CatalogMetadata`, Product Health Gold schema, QueryRouter/RAG/LLM/UI 적용 전략과 테스트 계획이 문서화되고, 전체 main merge 없이 파일별 이식 기준이 정리됨 |
| C-48A | `feature/frontend-shell-split` | `App.jsx`에서 shell/router/page composition을 분리하고 기능 동작은 유지한다 | C-48 완료 또는 사람 확인 | navigation, route normalization, app shell, standalone page entry가 분리되고 기존 주요 경로가 동일하게 렌더링됨 |
| C-48B | `feature/dataset-feature-boundary` | Dataset workspace의 Connection/Source/Silver/Gold/Jobs 상태와 action 경계를 feature 내부로 더 나눈다 | C-48A 완료 | `SourcesPage`의 거대 state/action 묶음이 도메인별 hook/model/action으로 분리되고 Dataset 주요 흐름이 회귀 없이 동작함 |
| C-49 | `feature/product-health-gold-lake-write-through` | Product Health Gold 수동 실행이 prepared reference를 참조하더라도 run별 `data/lake/gold` 산출물을 실제로 남기게 한다 | C-46 완료 또는 사람 확인 | 실행된 Gold Run의 `output_path`가 `data/lake/gold/run_id=<run_id>/...parquet`를 가리키고, prepared source path는 input/reference evidence로만 남음 |
| C-50 | `feature/product-health-lake-catalog-handoff` | Catalog publish와 AI Query가 `data/local_sources`가 아니라 방금 생성된 `data/lake/gold` run output을 읽게 한다 | C-49 완료 | CatalogDataset storage, lineage, AI Query selected dataset/evidence/retrieval trace가 같은 lake output path와 run id를 가리킴 |
| C-51 | `feature/manual-run-scheduler-boundary` | 수동 실행, schedule metadata, 실제 scheduler 미구현 경계를 UI/API/문서에서 일관되게 표시한다 | C-49 또는 C-50 완료 | Manual run은 실제 execute endpoint를 호출하고, schedule placeholder는 자동 실행 또는 Airflow/Spark DAG 성공처럼 보이지 않음 |
...
- C-48은 설계/계약 정렬 Phase다. `origin/main` 전체 merge를 전제로 하지 않고, main의 `QueryRouter`, `CatalogRetrievalIndex`, LLM adapter, AI Query UI 계약을 현재 브랜치의 Product Health runtime catalog 결과에 얹는 파일별 이식 계획을 만든다. `evidence.storage.local_fallback_path`는 UI/SQL handoff evidence로 유지하되 외부 LLM context에서는 제거해야 한다. `product_id`와 `internal_product_id` 호환 전략을 명시하고, 코드 적용은 후속 구현 Phase에서 진행한다.
- C-48A는 behavior-preserving frontend shell refactor다. `App.jsx`에서 app shell, navigation, route helpers, standalone page entry를 분리하되 API contract, 화면 copy, demo flow, data fetching behavior를 의도적으로 바꾸지 않는다.
- C-48B는 Dataset workspace 내부 경계 정리 Phase다. Connection/Source/Silver/Gold/Jobs/Runs의 state/action/model을 작은 hook 또는 module로 나누되 UI redesign, 새 API, C-49 lake write-through 구현은 포함하지 않는다.
- C-49는 Product Health happy path의 저장 의미를 닫는다. prepared `data/local_sources/product_health/gold/gold_product_health.parquet`를 사용할 수는 있지만, 성공한 Run의 output artifact는 항상 `data/lake/gold/run_id=<run_id>/...parquet`에 write-through 되어야 한다. prepared path는 input/reference evidence이고 Catalog publish 대상 output path가 아니다.
- C-50은 Catalog/AI Query handoff를 lake output 기준으로 다시 닫는다. CatalogDataset storage와 AI Query evidence가 `data/local_sources` prepared path를 최신 실행 결과처럼 가리키면 실패다.
- C-51은 실제 스케줄러를 구현하는 Phase가 아니다. `manual`은 사람이 누르는 execute이고, `placeholder`는 metadata 의도만 저장한다. Airflow/Spark scheduler, cron worker, retry/backfill, status polling 구현은 별도 runtime orchestration Phase로 둔다.
