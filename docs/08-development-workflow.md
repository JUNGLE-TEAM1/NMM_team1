...
- Auto PR Creation: when local validation, PR readiness, pre-merge/pre-PR sync checks, included/excluded file review, and stop-condition checks pass, AI may run final validation, feature branch push, and PR creation without additional human approval.
- Pre-PR Human Checkpoint: after PR creation, or when merge/finalize/cleanup/handoff is the next natural action, present a 2-4 option handoff menu before merge, finalize, issue close, branch cleanup, deploy, or next Phase handoff.
- PR Approval Scope: automatic PR creation authorizes only feature branch push and PR creation. A human choice such as `PR 진행`, `merge 진행`, or equivalent is required for CI/check follow-up that leads to merge, PR merge, finalize, issue close, branch cleanup, deploy, or external execution.
...
- Project Status Lifecycle: linked issue Project status follows `docs/11-git-sync-policy.md` (`In Progress` at branch start, `Review` at PR open, `Done` only after PR merge/finalize).
...
- AI records sync status and may run PR-ready feature branch push/PR creation automatically, but does not run pull, merge, rebase, PR merge, finalize, issue close, branch cleanup, deploy, or external execution without human confirmation.
- Auto PR Exception: complete PR-ready branch workspaces may create a PR automatically; silence or missing approval after PR creation means merge/finalize/cleanup is held.
...
Branch workspace가 `ready-for-review` 또는 `complete`에 가까워졌고 local validation과 PR readiness가 통과했으면, AI는 사람의 추가 승인 없이 final validation, feature branch push, PR 생성을 진행할 수 있다.
AI는 PR 생성 뒤 merge/finalize/branch handoff/integration handoff가 다음 자연스러운 행동이면 `Pre-PR Human Checkpoint`를 수행한다.

현재 상태에는 branch 이름, linked GitHub issue, PR closing keyword, PR link, local validation 결과, 남은 remote 작업, 외부 실행/배포/cloud approval 필요 여부를 포함한다.
...
1. PR 진행
   - 진행 절차: 이미 생성된 PR의 CI/check 확인 -> CI 통과 시 merge -> linked issue close 확인 -> `scripts/prepare-pr.sh --finalize <workspace>` -> 자동 merged branch cleanup -> GitHub 상태와 남은 branch queue 확인.
   - 선택하면 좋은 상황: 현재 PR을 main에 반영해도 되고 CI/리뷰 흐름을 마무리할 준비가 된 경우.
   - 장점: 변경이 main에 들어가 다음 Phase가 같은 기준에서 시작된다.
   - 주의사항 또는 단점: 원격 상태가 바뀐다. 사용자가 "PR만 올려줘", "머지는 보류"라고 하면 merge 전 멈춘다.
   - 원격/외부 상태 변경 여부: merge, issue close/finalize, local/remote Git branch cleanup이 포함된다.
2. 추가 보강
   ...
   - 원격/외부 상태 변경 여부: 기본 없음. merge/finalize/deploy가 필요해지면 별도 승인 필요.
3. 다음 Phase
...
4. 보류
   - 진행 절차: PR 생성 전이면 push/PR 없이 멈추고, PR 생성 후이면 열린 PR을 유지한 채 merge 없이 멈춘다. 보류 이유와 재개 조건을 `next-actions.md`에 기록한다.
...
사람이 `PR 진행`, `merge 진행`, 또는 동등한 지시를 명시하면 이미 생성된 PR의 CI 확인, merge, finalize, linked issue close 확인, merged branch cleanup까지 승인한 것으로 본다.
사람이 `PR 생성만`, `초안 PR`, `PR만 올려줘`, `머지는 보류`라고 제한하면 PR 생성까지만 유지하고 merge/finalize/cleanup 전 다시 멈춘다.
사람이 응답하지 않았거나 명시 승인이 없으면 PR 생성 이후 상태까지만 보고하고 merge/finalize/cleanup을 실행하지 않는다.
...
사람이 PR 생성 전에 `PR 올리지 마`, `로컬에만 둬`, `보류`, `PR은 나중에`, `draft만`이라고 명시하면 PR 생성을 하지 않는다.
