...
22. If the branch workspace is `complete`, pending confirmations are clear, and PR checklist is ready, AI presents a completion handoff choice menu before any remote action.
...
- 새 규칙을 추가하면 통과해야 하는 positive fixture와 실패해야 하는 negative fixture를 추가한다.
- 기존 규칙을 바꾸면 관련 fixture expected result를 업데이트한다.
...
- `scripts/test-harness.sh`를 실행하고 결과를 `quality.md`와 `report.md`에 기록한다.
- 실제 `gh issue create`, push, PR 생성, merge, deploy는 harness test에서 실행하지 않는다. 원격/외부 상태 변경은 dry-run 또는 local check로만 검증한다.
...
| M3. 소스와 카탈로그 | `feature/source-catalog` | CSV/local file source 등록, SQLite-backed `MetadataStore`, catalog 목록/상세 표시 | source metadata, schema/row count/sample, `MetadataStore` 교체 경계가 기록됨 |
