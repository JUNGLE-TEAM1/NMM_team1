...
Fast Path는 작은 저위험 변경을 빠르게 local complete 상태까지 닫기 위한 선택 경로다.
기존 Phase Workflow를 대체하지 않고, 아래 조건을 모두 만족할 때만 쓴다.

Fast Path 후보:

- 작은 문서, 테스트, UI copy, 로컬 버그 수정
- API, schema, data, security, permission, migration, deploy 영향 없음
- shared Source of Truth 변경이 없거나 영향이 작고 명확함
- 독립 기능 Phase나 integration Phase가 필요 없음
- 검증 명령 1-2개 또는 수동 확인으로 충분함

Fast Path 제외:

- API, schema, data, security, privacy, permission, migration 영향
- CI/CD workflow, harness behavior, scripts 변경
- dependency, runtime, Docker, deploy 변경
- integration branch, PR-ready, merge, remote 상태 변경
- shared contract나 Source of Truth의 의미 변경

Fast Path에서도 branch workspace는 유지한다.
다만 `plan.md`, `quality.md`, `sync.md`, `report.md` 중심으로 짧게 기록하고, `notes.md`, `sources.md`, `shared-docs.md`, `decisions.md`, `confirmations.md`, `next-actions.md`는 해당 없으면 `not needed` 또는 짧은 placeholder를 허용한다.
검증을 생략하면 `quality.md`에 이유와 대체 확인을 남긴다.

### Local Complete Gate

`local complete`와 `PR-ready`는 다른 상태다.

`local complete`는 로컬 구현, focused 검증, skip 이유 기록, 최소 report/quality 기록이 끝난 상태다.
작은 변경은 local complete로 보류할 수 있다.

`PR-ready`는 팀 공유 산출물로 올릴 준비가 끝난 상태다.
필요한 strict validation, sync, linked issue, PR body, CI 영향, required check 전략을 확인한 뒤에만 PR-ready로 승격한다.

local complete에서 PR-ready로 승격할 때는 `docs/10-next-action-menu.md`의 `Promote Local Complete To PR Ready` 메뉴를 사용한다.
...
- MongoDB Source Dataset seed slice는 C-1/C-2의 실사용 보강으로, local Docker MongoDB에 demo JSONL 일부를 적재하고 저장 전 collection schema preview 테스트와 Source Dataset metadata 저장까지만 닫는다.
